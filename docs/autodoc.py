...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    """Build the text of the file and write the file."""
    text = format_heading(1, makename(master_package, subroot))
    ...
    submods = [path.splitext(sub)[0]
               for sub in py_files
               if not shall_skip(
                   path.join(root, sub), opts) and sub != INITPY]
    # if there are some package directories, add a TOC for theses subpackages
    if submods:
        text += format_heading(2, 'Submodules')
        text += '.. toctree::\n    :maxdepth: 1\n\n'
        for mod in submods:
            text += '    %s.%s\n' % (makename(master_package, subroot), mod)
        text += '\n'
...
def recurse_tree(rootpath, excludes, opts):
    ...
    for root, subs, files in walk(rootpath, followlinks=followlinks):
        ...
        py_files = sorted(
            f
            for f in files
            if path.splitext(f)[1] in PY_SUFFIXES and not is_excluded(
                path.join(root, f), excludes))
        ...
        subs[:] = sorted(
            sub
            for sub in subs
            if not sub.startswith(exclude_prefixes) and not is_excluded(
                path.join(root, sub), excludes))
        ...
        if is_pkg:
            ...
            if subs or len(py_files) > 1 or not \
               shall_skip(path.join(root, INITPY), opts):
                ...
                subpackage_name = subpackage.replace(path.sep, '.')
                create_package_file(root, root_package, subpackage_name,
                                    py_files, opts, subs)
                toplevels.append(makename(root_package, subpackage_name))
                for py_file in py_files:
                    if not shall_skip(
                            path.join(rootpath, subpackage, py_file), opts):
                        module = path.splitext(py_file)[0]
                        create_module_file(root_package,
                                           '.'.join([subpackage_name, module]),
                                           opts)
